...
class Feediscount_model extends CI_Model {
    ...
    public function searchAssignFeeByClassSection($class_id = null, $section_id = null, $fees_discount_id = null, $category = null, $gender = null, $rte = null) {
        $sql = "SELECT IFNULL(`student_fees_discounts`.`id`, '0') as `student_fees_discount_id`,"
                ...
                . " `students`.`id`, `classes`.`class`, `division`.`id` AS `division_id`,"
                . " `division`.`division`, `students`.`id`, `students`.`admission_no`,"
                ...
                . " `student_session`.`class_id` = `classes`.`id` JOIN `division` ON"
                . " `division`.`id` = `student_session`.`division_id` LEFT JOIN `categories` ON"
